...
class RootCheckFragment : Fragment(R.layout.fragment_root_check) {
    ...
    private fun updateUi() {
        if (getRootAccess()) {
            ...
            if (getWriteSecureSettingsAccess()) {
                ...
                binding.textviewRootStatus.text =
                    "Your device is not rooted and WRITE_SECURE_SETTINGS is not granted."
                this@RootCheckFragment.context?.let { it1 ->
                    Toasty.info(
                        it1,
                        "Device not rooted. You have to grant <WRITE SECURE SETTINGS> PERMISSION.",
                        Toast.LENGTH_SHORT,
                        false
                    ).show()
                }
    ...
    private fun showTutorialDialog() {
        dialog = InfoSheet().build(requireActivity()) {
            ...
            content(
                "WRITE_SECURE_PERMISSION is a special permission that allows the app to modify the system's settings data.\n" +
